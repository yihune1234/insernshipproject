...
import { getVerifierNotifications, markVerifierNotificationRead, markAllVerifierNotificationsRead, deleteVerifierNotification } from '../../api/verifier';
...
export default function VerifierNotifications() {
  ...
  const markRead = async (id) => {
    try { await markVerifierNotificationRead(id); setNotifications((n) => n.map((x) => x.id === id ? { ...x, is_read: true } : x)); } catch { }
  };
  const markAll = async () => {
    try { await markAllVerifierNotificationsRead(); setNotifications((n) => n.map((x) => ({ ...x, is_read: true }))); toast.success('All read'); } catch { }
  };
  const remove = async (id) => {
    try { await deleteVerifierNotification(id); setNotifications((n) => n.filter((x) => x.id !== id)); } catch { }
  };
          ...
          {notifications.map((n) => (
            <div key={n.id} className={`flex items-start gap-3 rounded-xl border bg-card p-4 ${!n.is_read ? 'bg-primary/5 border-primary/20' : 'border-border'}`}>
              ...
              <div className="flex gap-1">
                ...
                <Button size="icon" variant="ghost" onClick={() => remove(n.id)}><Trash2 className="h-4 w-4 text-muted-foreground" /></Button>
              </div>
            </div>
          ))}
        </div>
